...
const ProductImage = ({ src, name }: { src: string; name: string }) => {
  ...
  return <img alt=ProductImage src={image.value.src} />;
}
```
...
```tsx
<Avatar.Root alt={user.name} className='avatar' src={user.imageUrl}>
...
const AvatarRoot = ({ alt, children, src, ...props }: AvatarRootProps) => {
...
const AvatarImage = (props: Omit<ImgHTMLAttributes<HTMLImageElement>, 'alt' | 'src'>) => {
...
const AvatarFallback = ({ children, ...props }: HTMLAttributes<HTMLSpanElement>) => {
...
- **`useImage` makes image loading explicit** - loading, success, and error become React state.
- **Render only when ready** - show skeletons or fallbacks without flashing broken browser UI.
- **Great fit for avatars** - `Root` owns image state, `Image` renders on success, `Fallback` renders during loading or failure.
- **The loaded value is useful** - `value` is an `HTMLImageElement`, ready for dimensions, canvas, and downloads.
